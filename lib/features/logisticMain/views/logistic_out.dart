...
class _LogisticOutState extends State<LogisticOut> {
                            ...
                            if (isExpired) {
                              ...
                              return Column(
                                children: [
                                  GestureDetector(
                                    onTap: (){
                                      Get.to(() => LogisticDetailsPage(data: _resultList[index], source: 'logistikOut'));
                                    },
                                    child: Card(
  ...
  DateTime _defaultSelectedStartDate = DateTime.parse('1945-01-01');
  ...
  getRecords()async{
    ...
    Query<Map<String, dynamic>> query = FirebaseFirestore.instance.collection('logistikKeluar');
    ...
    if (_selectedFilterOption2.isNotEmpty) {
      ...
      if (_selectedFilterOption2.length == 2){
        query
            .where('Tanggal Keluar', isGreaterThanOrEqualTo: _defaultSelectedStartDate)
            .where('Tanggal Keluar', isLessThanOrEqualTo: _defaultSelectedEndDate);
      }
    }
    ...
    for (var doc in logisticData.docs) {
      ...
      // Modify nanoseconds to 0
      resultWithId['Tanggal Keluar'] = Timestamp(resultWithId['Tanggal Keluar'].seconds, 0,);
  ...
  Future<void> generatePDF() async {
    ...
    const int firstPageRow = 3;
    const int otherPagesRow = 5;
